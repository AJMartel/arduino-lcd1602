...
void LiquidCrystal_1602_RUS::setCustomCharSize(uint8_t newsize) {
    lcd_char = new uint8_t*[newsize];

    index_lcd_char = new uint8_t[newsize];
    for (byte i = 0; i < 8; i++)
      index_lcd_char[i] = 255;

    lcd_char_n = 0;
}

void LiquidCrystal_1602_RUS::addCustomChar(uint8_t newchar[]) {

    lcd_char[lcd_char_n] = new uint8_t[8];
    for (byte i = 0; i < 8; i++) {
        lcd_char[lcd_char_n][i] = newchar[i];	
    }
    lcd_char_n++;
}
...
void LiquidCrystal_1602_RUS::print(int code, bool spec) {
    ...
    CharSetToLCD((uint8_t *)lcd_char[code], &index_lcd_char[code]);     
